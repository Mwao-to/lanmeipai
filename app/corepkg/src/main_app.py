...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; outline:none; }
  ...
  /* ---- 内置隐藏式进度条(位于播放器内部上方,点空白处显示/隐藏,收起时不占空间) ---- */
  .p-seekrow {
    display:flex; align-items:center; gap:10px;
    max-height:0; opacity:0; overflow:hidden; margin-bottom:0;
    transition:max-height .3s var(--ease-out), opacity .18s ease, margin-bottom .28s var(--ease-out);
  }
  .p-seekrow.open { max-height:26px; opacity:1; margin-bottom:9px; }
  ...
  @media (max-width:600px) {
    .p-seekrow.open { margin-bottom:7px; }
...
<div class="player">
  <div class="player-card" onclick="toggleSeek(event)">
    <div class="p-seekrow" id="seekRow">
...
/* ---- 点播放器空白处:显示/隐藏内部上方的进度条(播放/下载等按钮点击不触发) ---- */
function toggleSeek(e) {
  if (e.target.closest('button')) return;
  $('seekRow').classList.toggle('open');
}
