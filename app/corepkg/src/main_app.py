...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; outline:none; }
  ...
  /* ═══ C键·EQ弹窗(复用modal结构;竖向预设列表+8柱dB迷你图) ═══ */
  .eq-list {
    margin-top:12px; max-height:52vh; overflow-y:auto; overscroll-behavior:contain;
    border-top:1px solid var(--glass-border);
  }
  .eq-row {
    display:flex; align-items:center; gap:10px;
    padding:9px 6px; border-bottom:1px solid rgba(255,255,255,.05);
  ...
  .eq-bars {
    flex:1; height:20px; display:flex; align-items:flex-end;
    justify-content:flex-end; gap:2px;
  }
  .eq-bars i { width:5px; background:rgba(255,255,255,.18); border-radius:1px; }
  .eq-row.on .eq-bars i { background:var(--accent-hover); }
 ...
 * Hz底子固定:100/200/400/800/1K/3K/6K/10K;dB每段可调,硬限±8。
...
const EQ_PRESETS = {
  ...
  'DJ音效':   [7,6,2,-1,-2,2,5,6],
  '经典流行': [3,2,0,-1,0,2,3,3],
  '爵士':     [3,2,1,2,1,0,1,2],
  '古典':     [2,1,0,0,0,1,2,3],
  '高音增强': [0,0,0,0,1,3,6,8],
...
async function applyEq(name) {
  ...
  if (!vals) return;
  if (name !== '正常' && !eqCtx && !ensureEqChain()) return;
  if (eqCtx) {
    if (eqCtx.state === 'suspended') { try { await eqCtx.resume(); } catch (e) { } }
    vals.forEach((db, i) => { eqFilters[i].gain.value = clampDb(db); });   // dB硬限±8
  }
  eqActiveName = name;
...
function renderEqList() {
  $('eqList').innerHTML = Object.keys(EQ_PRESETS).map(n => {
    const bars = EQ_PRESETS[n]
      .map(v => `<i style="height:${Math.max(6, Math.round((clampDb(v) + 8) / 16 * 100))}%"></i>`).join('');
    return `<div class="eq-row${n === eqActiveName ? ' on' : ''}" onclick="applyEq('${n}')">`
      + `<span class="eq-name">${n}</span><span class="eq-bars" title="100/200/400/800/1K/3K/6K/10K Hz">${bars}</span></div>`;
  }).join('');
}
function openEqModal() {
  renderEqList();
  $('eqModal').classList.add('show');
  if (!eqCtx && eqActiveName !== '正常') toast(`上次EQ「${eqActiveName}」待生效：点击该预设即启用`, 'info', 2500);
}
