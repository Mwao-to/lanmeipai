...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; outline:none; }
  ...
  .eq-name {                               /* 字号与歌单条目名称(pl-name)同款统一 */
  ...
  .eq-mark { margin-left:auto; flex-shrink:0; font-family:var(--mono); font-size:10.5px; color:#55555c; }   /* ㉿标记,字号与名称统一 */
...
(function restoreEq() {                    // 恢复上次预设:私有目录eqw.json优先,localStorage兑底
  ...
  if (eqActiveName !== '正常') {           // 上次用了预设:本次启动自动沿用,弹提示告知
    ...
    setTimeout(() => toast(`当前使用EQ：${eqActiveName}，已生效`, 'success', 2600), 600);
  }
})();
function maybeAutoBuildEq() {              // 播放路径上确保启动遗留的自动预设链就绪并填参
  if (!pendingAutoEq || eqCtx) return;
  if (!ensureEqChain()) { pendingAutoEq = null; return; }
...
function renderEqList() {
  $('eqList').innerHTML = Object.keys(EQ_PRESETS).map(n => {
    return `<div class="eq-row${n === eqActiveName ? ' on' : ''}" onclick="applyEq('${n}')">`
      + `<span class="eq-name">${n}</span><span class="eq-mark">㉿</span></div>`;
  }).join('');
}
