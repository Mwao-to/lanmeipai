...
(function loadImportedPresets() {          // 合并到可选预设表:重名自动加#2后缀,显示名保持原名
  let arr = null;
  try { const s = window.AndroidBridge && AndroidBridge.eqImport ? AndroidBridge.eqImport() : ''; if (s) arr = JSON.parse(s); } catch (e) { }
  if (!Array.isArray(arr)) arr = EQ_IMPORT_RAW;
  const used = new Set(Object.keys(EQ_PRESETS));
  arr.forEach(p => {
